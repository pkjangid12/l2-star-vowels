...
export default class LevelSelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    const logo = this.add.image(centerX, centerY * 0.37, "preload-logo");

    const instruction = this.add.image(
      centerX,
      centerY * 0.85,
      "gameInctructions",
    );

    const bunny = this.add.image(centerX, centerY * 1.4, "bunny");
    ...
    const button = this.add.image(centerX, height - 220, "playBtn").setScale(1);
